...
fn fetch_from_supabase(date: &str) -> Vec<RollRecord> {
    ...
    let table = std::env::var("SUPABASE_ROLLS_TABLE")
        .unwrap_or_else(|_| "dice_rolls".to_string());
        ...
        .map(|row| {
            ...
            RollRecord {
                ...
                results: serde_json::from_value(results)
                    .expect("could not parse 'results' column"),
                total: row.get("total"),
                label: row.get("label"),
            }
        })
...
fn main() {
    ...
    let file = std::fs::File::create(out_path).expect("could not create session_analysis_output.txt");
    ...
    match date_arg.as_deref() {
        Some(date) => writeln!(out, "Source: Supabase  Date: {}  ({} rolls)\n", date, rolls.len()).unwrap(),
    ...
    writeln!(out, "  Above average:  {:3}  ({:.1}%)", player_counts[0], pct(player_counts[0], p_total)).unwrap();
    writeln!(out, "  Average:        {:3}  ({:.1}%)", player_counts[1], pct(player_counts[1], p_total)).unwrap();
    writeln!(out, "  Below average:  {:3}  ({:.1}%)", player_counts[2], pct(player_counts[2], p_total)).unwrap();

    writeln!(out, "\nGM / {} ({} rolls):", gm_name, g_total).unwrap();
    writeln!(out, "  Above average:  {:3}  ({:.1}%)", gm_counts[0], pct(gm_counts[0], g_total)).unwrap();
    writeln!(out, "  Average:        {:3}  ({:.1}%)", gm_counts[1], pct(gm_counts[1], g_total)).unwrap();
    writeln!(out, "  Below average:  {:3}  ({:.1}%)", gm_counts[2], pct(gm_counts[2], g_total)).unwrap();
    ...
    for name in &player_order {
        ...
        writeln!(out, "  Above average:  {:3}  ({:.1}%)", counts[0], pct(counts[0], total)).unwrap();
        writeln!(out, "  Average:        {:3}  ({:.1}%)", counts[1], pct(counts[1], total)).unwrap();
        writeln!(out, "  Below average:  {:3}  ({:.1}%)", counts[2], pct(counts[2], total)).unwrap();
        writeln!(out).unwrap();
    }
