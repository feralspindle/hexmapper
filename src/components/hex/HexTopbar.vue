<template>
  <header class="ds-topbar" style="gap:0">
    ...
    <RouterLink to="/" class="ds-brand" style="flex-shrink:0;text-decoration:none">
      <svg class="ds-brand-mark" width="28" height="28" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
        <rect width="32" height="32" rx="6" fill="#1c1917"/>
        <polygon points="16,3.5 27.5,9.75 27.5,22.25 16,28.5 4.5,22.25 4.5,9.75" fill="#1c1917" stroke="#d4a74b" stroke-width="2" stroke-linejoin="round"/>
    ...
    <div class="ds-session-name" style="cursor:default" @click="startEdit" v-if="!editing">
    ...
    <div v-if="hexMode && sessionStore.isGM" class="hm-mode-badge" :data-mode="hexMode">
      <span class="hm-mode-label">{{ hexMode === 'fow' ? 'Fog of War' : 'Blank Slate' }}</span>
    ...
    <ShareModal :session-id="sessionStore.sessionId" style="margin-right:4px" />
    <BugReportButton style="margin-right:10px" />

    <div style="display:flex;align-items:center;padding-left:10px;border-left:1px solid rgba(237,225,199,.15);gap:8px;flex-shrink:0">
      <img
        v-if="authStore.avatarUrl && !avatarErr"
        :src="authStore.avatarUrl"
        :alt="authStore.displayName"
        style="width:24px;height:24px;border-radius:50%;object-fit:cover;border:1px solid rgba(237,225,199,.25)"
        @error="avatarErr = true"
      />
      <div
        v-else
        style="width:24px;height:24px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:12px;color:var(--paper)"
      >{{ authStore.displayName?.charAt(0)?.toUpperCase() }}</div>
      <span style="font-family:var(--font-display);font-size:12px;color:rgba(237,225,199,.7);max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
        {{ authStore.displayName }}
      </span>
      <button
        style="background:none;border:none;color:rgba(237,225,199,.45);font-size:16px;cursor:pointer;padding:0;line-height:1"
        title="Sign out"
...
function commitName() {
  editing.value = false
  if (nameInput.value.trim()) sessionStore.updateSessionName(nameInput.value.trim())
}
function cancelEdit() {
  editing.value = false
}
