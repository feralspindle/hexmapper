<template>
    ...
    >
        <RouterLink
            to="/"
            class="text-parchment-300 hover:text-parchment-100 font-display text-lg transition-colors"
            >⬡ Hex Map</RouterLink
        >
        ...
        <div v-if="onlineOthers.length" class="flex items-center gap-1.5">
            ...
            >
                ...
                <div
                    class="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 px-2 py-1 bg-stone-900 border border-stone-700 rounded text-sm text-stone-200 whitespace-nowrap pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50"
                >
                    {{ viewer.display_name }}
                </div>
            </div>
        ...
        <div class="ml-auto flex items-center gap-3">
            ...
            >
                <i
                    :class="charOpen ? 'fa-solid fa-xmark' : 'fa-solid fa-user'"
                />
                <span class="hidden sm:inline">{{
                    charOpen ? "Close Character Sheet" : "Character Sheet"
                }}</span>
            </button>
            ...
            <template v-if="authStore.isAuthenticated">
                <div
                    class="flex items-center gap-2 pl-2 border-l border-stone-700"
                >
                    <img
                    ...
                    <span class="text-stone-300 text-sm truncate max-w-28">{{
