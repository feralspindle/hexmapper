...
describe('notesStore', () => {
  ...
  test('realtime events dedupe by id and apply INSERT/UPDATE/DELETE regardless of author', async () => {
    kit.responses.hex_notes = { data: [note('n1')], error: null }
    const store = useNotesStore()
    await store.initForHex('hex-1', 's1')
    const channel = kit.channels[0]

    channel.emitPostgres('hex_notes', 'INSERT', note('n1'))
    expect(store.notes.map(n => n.id)).toEqual(['n1'])
  ...
  test('the realtime echo of your own freshly added note never duplicates it', async () => {
    ...
    kit.channels[0].emitPostgres('hex_notes', 'INSERT', note('server-note', { body: 'hello', user_id: 'me' }))

    expect(store.notes.filter(n => n.id === 'server-note')).toHaveLength(1)
  })
