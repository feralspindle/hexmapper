-- =============================================================================
-- Server-side dice rolling: all RNG happens in Postgres, results are
-- server-authoritative. Clients can no longer INSERT into dice_rolls directly.
-- =============================================================================
...
as $$
declare
  v_allowed_dice text[]  := array['d4','d6','d8','d10','d12','d20','d100'];
  v_die          text;
  v_sides        int;
  v_count        int;
  v_i            int;
...
begin
  ...
  -- Insert row; display_name is overwritten by fill_display_name trigger
...
-- Drop the direct-insert policy: clients must use roll_dice() RPC from now on.
-- The RPC is security definer and handles the insert itself.
